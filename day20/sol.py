...
#file = open("day20/input_2023_20.txt").readlines()
file = open("input_2023_20.txt").readlines()
...
for name, module in modules.items():
    for destination in module.destinations:
        if destination in modules and modules[destination].type == ModuleType.CONJUCTION:
            modules[destination].memory[name] = PulseType.LOW

lengths = {}
visited = {name: 0 for name, module in modules.items() if rx_feeder in module.destinations}
count = 0

while True:
    count += 1
    queue = deque([("broadcaster", x, PulseType.LOW) for x in modules["broadcaster"].destinations])
